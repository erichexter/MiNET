...
using MiNET.Utils;
using MiNET.Worlds.BiomeSystem;
using MiNET.Worlds.Structures;

namespace MiNET.Worlds
{
	public class BiomeCoolWorldProvider : IWorldProvider
	{
		...
		private void PopulateChunk(ChunkColumn chunk)
		{
			...
			iBiome biome = new BiomeFinder(_seed.GetHashCode()).GetBiome(chunk.x, chunk.z);
			overhangsMagnitude = biome.OverhangsMagnitude;
			bottomsMagnitude = biome.BottomsMagnitude;
			...
			for (int x = 0; x < 16; x++)
			{
				for (int z = 0; z < 16; z++)
				{
					...
					chunk.SetBlock(x, bottomHeight, z, biome.TopBlock.Id); //the top of the base hills
		...
		private void GenerateTree(ChunkColumn chunk, int x, int treebase, int z, iBiome biome)
		{
			//new OakTree().Create(chunk, x, treebase, z);
			biome.TreeStructure.Create(chunk, x, treebase, z);
